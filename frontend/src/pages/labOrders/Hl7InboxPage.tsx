...
export function Hl7InboxPage() {
  ...
  async function performLookup(accNo: string) {
    ...
    setLoading(true)
    try {
      const res = await getResultByAccession(accNo.trim())
      setResult(res)
    } catch {
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
              <div className="flex items-start justify-between flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-mono mt-0.5">MRN: {result.mrn}</p>
                </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {result.observations.map((obs) => (
                      <tr key={obs.labObservationId} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-2.5 font-bold text-gray-800">{obs.resultValue}</td>
                        <td className="px-4 py-2.5 text-gray-500 font-medium">{obs.unit ?? '—'}</td>
                        <td className="px-4 py-2.5 text-gray-500 font-mono text-[10px]">{obs.referenceRange ?? '—'}</td>
                        <td className="px-4 py-2.5 text-center">
                          {obs.flag ? (
                            <span className={`px-1.5 py-0.5 rounded text-[9px] font-extrabold ${FLAG_COLORS[obs.flag] ?? 'text-gray-600'}`}>
                              {obs.flag}
                            </span>
                          ) : (
