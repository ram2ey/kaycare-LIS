...
import type { LabOrderSummary, LabOrderDetail } from '../types/labOrders'
...
export function PatientTimeline({
  ...
  const timelineEvents: TimelineEvent[] = [
    ...appointments.map((a) => ({
      ...
      subtitle: a.reason || 'No reason specified',
      rawItem: a,
    })),
    ...bills.map((b) => ({
      id: b.billId,
    ...
    ...labOrders.map((o) => ({
      ...
      title: `Lab Order (${o.accessionNumber || 'No Accession'})`,
      status: o.status,
      statusColor: LAB_STATUS_COLORS[o.status] ?? 'bg-gray-100 text-gray-600',
      priority: o.priority,
                          ...
                          <div className="bg-gray-50/70 border border-gray-150 rounded-xl px-4 py-3 text-xs grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <div>
                              ...
                              <span className="font-mono font-bold text-gray-700">
                                {labOrderDetails[event.id].accessionNumber || '—'}
                              </span>
                            </div>
                            {labOrderDetails[event.id].clinicalNotes && (
                              <div>
                                <span className="text-gray-400 font-semibold block">Clinical Notes</span>
                            ...
                            <table className="w-full text-xs text-left">
                              <thead>
                                <tr className="text-gray-500 border-b border-gray-100 bg-gray-50 font-semibold">
                                  <th className="px-4 py-2.5">Test Code / Name</th>
                              ...
                              <tbody className="divide-y divide-gray-50">
                                {labOrderDetails[event.id].items.map((item) => {
                                  const resultVal = item.hl7ResultValue ?? item.manualResultValue
                                  const resultUnit = item.hl7ResultUnit ?? item.manualResultUnit
                                  const refRange = item.manualResultReferenceRange
                                  const flag = item.hl7Flag ?? item.manualResultFlag

                                  return (
                                    <tr key={item.labOrderItemId} className="hover:bg-gray-50/30">
                                      <td className="px-4 py-2.5">
                                        <p className="font-bold text-gray-800">{item.testName}</p>
                                        <p className="text-[9px] text-gray-400 font-mono mt-0.5">{item.testCode}</p>
                                      </td>
                                      <td className="px-4 py-2.5">
                                      ...
                                      <td className="px-4 py-2.5 font-bold text-gray-800">{resultVal ?? '—'}</td>
                                      ...
                                      <td className="px-4 py-2.5 text-gray-500">
                                        {item.signedByName ? (
                                          <div>
                                            <p className="font-semibold text-gray-700">{item.signedByName}</p>
                                            <p className="text-[9px] text-gray-400">
                                              {item.signedAt ? new Date(item.signedAt).toLocaleDateString('en-GB') : ''}
                                            </p>
                                          </div>
                                        ) : (
                                          <span className="text-gray-300">—</span>
