...
public class LabResultConfiguration : IEntityTypeConfiguration<LabResult>
{
    public void Configure(EntityTypeBuilder<LabResult> builder)
    {
        ...
        builder.Property(r => r.Status).HasMaxLength(20).IsRequired().HasDefaultValue(LabResultStatus.Received);
        ...
        builder.HasOne(r => r.Patient).WithMany().HasForeignKey(r => r.PatientId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne(r => r.OrderingDoctor).WithMany().HasForeignKey(r => r.OrderingDoctorUserId).OnDelete(DeleteBehavior.Restrict);
        builder.HasMany(r => r.Observations).WithOne(o => o.LabResult).HasForeignKey(o => o.LabResultId).OnDelete(DeleteBehavior.Cascade);
    }
}
