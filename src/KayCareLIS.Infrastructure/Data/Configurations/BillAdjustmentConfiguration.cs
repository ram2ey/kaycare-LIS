...
public class BillAdjustmentConfiguration : IEntityTypeConfiguration<BillAdjustment>
{
    public void Configure(EntityTypeBuilder<BillAdjustment> builder)
    {
        ...
        builder.HasOne(a => a.Bill).WithMany(b => b.Adjustments).HasForeignKey(a => a.BillId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(a => a.AdjustedBy).WithMany().HasForeignKey(a => a.AdjustedByUserId).OnDelete(DeleteBehavior.Restrict);
